...
class CropTypeMapping(RawGeoFMDataset):
    def __init__(
        self,
        split: str,
        dataset_name: str,
        ...
        """
        super(CropTypeMapping, self).__init__(
            split=split,
        ...
        assert country in ["ghana", "southsudan"]
        self.country = country
        self.use_pad = use_pad

        self.centroids = {
            "ghana":       {"lat": 7.95, "lon": -1.03},
            "southsudan": {"lat": 7.00, "lon": 30.00}
        }
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            indexes = torch.linspace(0, s2.shape[-1] - 1, self.multi_temporal, dtype=torch.long)
            s2 = s2[..., indexes]
            ...
            doy_norm = (metadata % 365) / 365
            lat_norm = torch.tensor(self.centroids[self.country]["lat"], dtype=torch.float32) / 90.0
            lon_norm = torch.tensor(self.centroids[self.country]["lon"], dtype=torch.float32) / 180.0
    ...
    def get_metadata(self, idx):
        ...
        indexes = torch.linspace(0, s2.shape[0] - 1, self.multi_temporal, dtype=torch.long)
        s2 = s2[indexes]
