...
class CropChileClassificationDataset(RawGeoFMDataset):
    ...
    def __init__(
        ...
        super(CropChileClassificationDataset, self).__init__(
            split=split,
    ...
    def __getitem__(self, i: int) -> dict:
        ...
        for mod, (directory, prefix) in static_cfg.items():
            ...
            if arr is None:
                image[mod] = torch.zeros(1, 1, self.img_size, self.img_size)
            else:
                image[mod] = torch.from_numpy(arr).unsqueeze(1)
        ...
        if label is not None:
            ...
            shifted_classes = valid_classes - 1
            
            shifted_classes = shifted_classes[
                (shifted_classes >= 0) & (shifted_classes < self.num_classes)
            ]
            
            if len(shifted_classes) > 0:
                target[shifted_classes] = 1.0
