...
class AsymmetricLossOptimized(nn.Module):
    ...
    def forward(self, x, y):
        # 1. Variables locales (se destruyen al final de la función liberando VRAM)
        xs_pos = torch.sigmoid(x)
        xs_neg = 1.0 - xs_pos

        # 2. Clipping in-place (ahorra memoria sin crear nuevos tensores)
        if self.clip is not None and self.clip > 0:
            xs_neg.add_(self.clip).clamp_(max=1)

        # 3. Cálculo de la pérdida cruzada básica
        los_pos = y * torch.log(xs_pos.clamp(min=self.eps))
        los_neg = (1 - y) * torch.log(xs_neg.clamp(min=self.eps))
        loss = los_pos + los_neg

        # 4. Modulación Asimétrica
        if self.gamma_neg > 0 or self.gamma_pos > 0:
            ...
            asymmetric_w = torch.pow(1 - pt, one_sided_gamma)
            
            if self.disable_torch_grad_focal_loss:
                # Restaurar al estado original (si era validación, volverá a False)
